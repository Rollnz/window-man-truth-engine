-- EMQ 9.5+ Schema Upgrade: Add missing address and device fields
...
-- Add index for common queries on zip
...
-- Add indexes for wm_leads
CREATE INDEX IF NOT EXISTS idx_wm_leads_zip ON public.wm_leads(zip);
CREATE INDEX IF NOT EXISTS idx_wm_leads_state ON public.wm_leads(state);

-- Update the handle_new_lead_to_crm trigger function to sync city/state/zip
...
BEGIN
  ...
  -- Validate session ID format (must be valid UUID or null)
