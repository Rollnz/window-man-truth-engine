 ...
 * We call the Supabase Edge Function directly (not /api/*) because Vite doesn't proxy functions.
...
const EDGE_BASE = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/wm-api`;
const EDGE_HEADERS = {
...
const createOrRefreshSession = async (): Promise<string> => {
  ...
  try {
    const res = await fetch(`${EDGE_BASE}/session`, {
      method: "POST",
