...
const QuoteBuilderHero \= () \=\> {  
  ...
  return (  
    \<section className="relative w-full min-h-\[50vh\] flex flex-col items-center justify-center bg-white overflow-hidden border-b border-slate-100"\>  
      \<div className="absolute inset-0 bg-\[radial-gradient(ellipse\_at\_top,\_var(--tw-gradient-stops))\] from-blue-50 via-white to-white" /\>  
        ...
        \<div className="p-6 overflow-y-auto"\>  
          {isLoading ? (  
            ...
            \<div className="prose prose-slate max-w-none text-slate-600"\>  
               \<div dangerouslySetInnerHTML={{ \_\_html: content.replace(/\\n/g, '\<br /\>') }} /\>  
            \</div\>  
          )}  
        \</div\>  
...
const QuoteBuilderV2 \= () \=\> {  
  ...
  const styleOptions \= state.productType \=== "window" ? STYLE\_OPTIONS.window : STYLE\_OPTIONS.door;  
  const sizeOptions \= state.productType \=== "window" ? SIZE\_OPTIONS.window : SIZE\_OPTIONS.door;
  ...
  const addToCart \= () \=\> {  
    ...
    const productConfig \= PRODUCT\_TYPE\_CONFIG\[state.productType\];  
                        ...
                        \<div className="space-y-4"\>  
                            ...
                            \<div className="grid grid-cols-2 gap-4"\>  
                                ...
                                \<div\>  
                                    ...
                                    \>  
                                        {WALL\_OPTIONS.map(opt \=\> \<option key={opt.value} value={opt.value}\>{opt.label}\</option\>)}  
                                    \</select\>  
                                \</div\>  
                            \</div\>  
                        \</div\>  
                                        ...
                                        {CONFIG.frameMaterials.map((frame, frameIdx) \=\> {  
                                            ...
                                            return (  
                                                \<div key={frame} className="grid grid-cols-4 gap-2 items-stretch"\>  
                                                    \<div className="flex items-center font-medium text-sm text-slate-600 pl-2"\>  
                                                        {frame}  
                                                    \</div\>  
                                                    {rowPrices.map((price, glassIdx) \=\> {  
                                                        const isSelected \= frameIdx \=== state.selectedFrameIndex && glassIdx \=== state.selectedGlassIndex;  
                        ...
                        \<div className="space-y-5"\>  
                            ...
                            \<div className="grid grid-cols-2 gap-4"\>  
                                \<div\>  
                                    ...
                                    \>  
                                        {COLOR\_OPTIONS.map(opt \=\> \<option key={opt.label} value={opt.value}\>{opt.label}\</option\>)}  
                                    \</select\>  
                                \</div\>  
                                \<div\>  
                                    ...
                                    \>  
                                        {GRID\_OPTIONS.map(opt \=\> \<option key={opt.label} value={opt.value}\>{opt.label}\</option\>)}  
                                    \</select\>  
                                \</div\>  
                            \</div\>
