...
import { useDataLayerMonitor, normalizeId, type SystemHealth, type AutopilotInsight, type HandshakeResult, type ParityResult, type ParityState, type MonitorEvent } from '@/hooks/useDataLayerMonitor';
...
function SystemHealthGauge({
  ...
  const config: Record<SystemHealth, { bg: string; icon: React.ReactNode; defaultMsg: string }> = {
    ...
    critical: {
      ...
      icon: <AlertOctagon className="h-6 w-6 text-destructive animate-pulse" />,
      defaultMsg: '',
    },
  };
...
// ─── Step 9: Budget Alert Banner ─────────────────────────────────────────────

function BudgetAlertBanner({ lostLeadRate }: { lostLeadRate: number }) {
  const [copied, setCopied] = useState(false);

  const handleCopyAlert = useCallback(async () => {
    const alert = `🚨 BUDGET PROTECTION ALERT\n\nTimestamp: ${new Date().toISOString()}\nLost Lead Rate: ${lostLeadRate}%\n\nAction Required:\n- Immediately pause all Google Ads campaigns\n- Immediately pause all Meta Ads campaigns\n- Check save-lead edge function logs for errors\n- Verify database connectivity and RLS policies\n\nThis alert was generated by the Truth Engine Guardian system.`;
    await copyToClipboard(alert);
    setCopied(true);
  ...
  return (
    <div className="rounded-lg border-2 border-destructive bg-destructive/10 p-6 space-y-4 animate-pulse-slow">
      <div className="flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h2 className="text-lg font-bold text-destructive">BUDGET PROTECTION ALERT</h2>
          <p className="text-sm text-foreground">
            {lostLeadRate}% of recent leads are failing to reach the database. Your advertising budget is at risk of being wasted on untracked conversions.
          </p>
        </div>
      </div>
      <div className="flex flex-wrap gap-3">
        ...
        >
          ...
          {copied ? 'Copied!' : 'Copy Emergency Alert'}
        </Button>
      </div>
    </div>
  );
...
function AutopilotCard({ insights }: { insights: AutopilotInsight[] }) {
  ...
  const severityIcon = (severity: AutopilotInsight['severity']) => {
    switch (severity) {
      case 'critical': return <AlertOctagon className="h-4 w-4 text-destructive shrink-0" />;
      case 'warning': return <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 shrink-0" />;
      default: return <AlertCircle className="h-4 w-4 text-blue-600 dark:text-blue-400 shrink-0" />;
    }
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
              ...
              {isExpanded && (
                <div className="px-3 pb-3 space-y-3 border-t pt-3">
                  ...
                  <div className="rounded-md bg-muted p-3">
                    <p className="text-xs font-medium mb-1">Proposed Fix:</p>
                    <p className="text-xs text-muted-foreground">{insight.proposedFix}</p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
        ...
        <p className="text-xs text-muted-foreground pt-2">
          {insights.length} insight(s) detected. {insights.filter((i) => i.severity !== 'info').length} actionable fix(es) available.
        </p>
      </CardContent>
    </Card>
  );
...
function TruthEngineCertification({ handshakeResults }: { handshakeResults: HandshakeResult[] }) {
  ...
  return (
    <div className="rounded-lg border border-border/50 bg-muted/30 p-6 text-center space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        All 10 Steps of the Deduplication &amp; Attribution Pipeline are Active.
      </p>
      {verifiedSpend > 0 && (
        <p className="text-xs text-muted-foreground">
          This session has verified <span className="font-mono font-semibold text-foreground">${verifiedSpend}</span> of projected conversion value.
        </p>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="container max-w-5xl py-8 space-y-6">
        ...
        {/* ═══ Step 9: Budget Alert ═══ */}
        {isCritical && <BudgetAlertBanner lostLeadRate={lostLeadRate} />}
        ...
        {monitorState.isMonitoring && (
          <>
            ...
            {/* ═══ Step 10: Autopilot ═══ */}
            <AutopilotCard insights={autopilotInsights} />
