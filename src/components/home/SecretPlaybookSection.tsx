...
interface RiskCardProps { number: number; title: string; subtitle: string; description: string; icon: React.ReactNode; iconColor: string; }

function RiskCard({ number, title, subtitle, description, icon, iconColor }: RiskCardProps) {
  return (
    <div className="group relative p-6 rounded-xl bg-card border border-border/50 hover:border-primary/30 transition-all duration-300 hover:translate-y-[-4px] hover:shadow-lg">
...
export function SecretPlaybookSection() {
  const risks = [
    { title: 'The Scope Gap', subtitle: 'The Hidden Back-Bill', description: "Our AI unearths what's quietly excluded — permits, debris removal, stucco or drywall repair — the \"missing middle\" that often becomes a $2,000+ surprise once work begins.", icon: <Calculator className="w-6 h-6 text-[hsl(var(--secondary))]" />, iconColor: "bg-[hsl(var(--secondary)/0.1)]" },
    { title: 'Code-Compliance Traps', subtitle: 'The Insurance Killer', description: '"Impact-rated" is marketing language. Our AI verifies whether the Design Pressure actually meets the legal requirements for your exact zip code — the difference between passing inspection and losing coverage.', icon: <ShieldOff className="w-6 h-6 text-red-500" />, iconColor: "bg-red-500/10" },
    { title: 'Paper Warranties', subtitle: 'The Man-in-a-Van Risk', description: "We expose labor exclusions and short warranty terms that vanish if the installer disappears. If something fails in year three, are you protected — or on your own?", icon: <FileQuestion className="w-6 h-6 text-amber-500" />, iconColor: "bg-amber-500/10" },
    { title: 'Risk-Shifting Clauses', subtitle: 'The Bait-and-Switch', description: "We neutralize fine-print clauses that allow substitutions, delays, or cost increases without your consent.", icon: <FileWarning className="w-6 h-6 text-purple-500" />, iconColor: "bg-purple-500/10" },
    { title: 'The Ignorance Tax', subtitle: 'Price Fairness', description: "We benchmark your quote against real local market data. Are you paying fair value — or a quiet 30% premium because no one expected you to check?", icon: <AlertTriangle className="w-6 h-6 text-primary" />, iconColor: "bg-primary/10" },
  ];
  ...
  return (
    <section className="py-20 md:py-32 relative bg-background overflow-hidden">
      ...
      <div className="container px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto mb-16">{risks.map((risk, index) => (<RiskCard key={index} number={index + 1} title={risk.title} subtitle={risk.subtitle} description={risk.description} icon={risk.icon} iconColor={risk.iconColor} />))}</div>
