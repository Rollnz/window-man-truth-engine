...
function RiskCard({
  ...
  return <div className="group relative p-6 rounded-xl transition-all duration-300 hover:translate-y-[-4px] hover:shadow-lg border-2 border-primary bg-primary/5">
      ...
      <h4 className="font-semibold mb-1 text-base text-foreground">{title}</h4><p className="text-sm font-medium text-[hsl(var(--secondary))] mb-3">{subtitle}</p><p className="text-sm text-muted-foreground leading-relaxed">{description}</p>
    </div>;
}
export function SecretPlaybookSection() {
  ...
  return <section className="py-20 md:py-32 relative bg-gradient-to-b from-muted/50 to-muted/30 overflow-hidden isolate pattern-dots">
      ...
      <div className="container px-4 relative z-10">
        <div className="max-w-3xl mx-auto text-center mb-16 text-base font-medium text-inherit"><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--secondary)/0.1)] border border-[hsl(var(--secondary)/0.2)] mb-6"><Sparkles className="w-4 h-4 text-[hsl(var(--secondary))]" /><span className="text-sm font-medium text-[hsl(var(--secondary))]">The AI Advantage</span></div><h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6">The "Secret Playbook" Hidden<span className="block mt-2 text-[hsl(var(--secondary))]">Inside Your Quote</span></h2><p className="text-lg text-muted-foreground">Contractors don't usually lie — they just omit.<br />Our AI de-codes the silent risks that turn a good-looking estimate into a financial nightmare.</p></div>
