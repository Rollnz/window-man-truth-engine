...
export function SecretPlaybookSection() {
  ...
  return <section className="py-20 md:py-32 relative bg-gradient-to-b from-[hsl(210,28%,96.8%)] to-[hsl(32,24%,97.2%)] overflow-hidden isolate pattern-dots">
      ...
      <div className="container px-4 relative z-10">
        <div className="max-w-3xl mx-auto text-center mb-16 text-[#013f9d] text-base font-medium"><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--secondary)/0.1)] border border-[hsl(var(--secondary)/0.2)] mb-6"><Sparkles className="w-4 h-4 text-[hsl(var(--secondary))]" /><span className="text-sm font-medium text-[hsl(var(--secondary))]">The AI Advantage</span></div><h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6">The "Secret Playbook" Hidden<span className="block mt-2 text-[hsl(var(--secondary))]">Inside Your Quote</span></h2><p className="text-lg text-muted-foreground">Contractors don't usually lie — they just omit.<br />Our AI de-codes the silent risks that turn a good-looking estimate into a financial nightmare.</p></div>
        ...
        <div className="max-w-3xl mx-auto mb-12"><div className="p-8 md:p-10 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 border border-white/10"><h3 className="text-xl md:text-2xl font-bold text-white text-center mb-6">The Confidence Shift</h3><p className="text-lg text-slate-100 text-center mb-4">With this report, you don't go back to a contractor asking questions.</p><p className="text-xl text-white font-semibold text-center mb-6">You go back with <span className="text-sky-600">answers</span>.</p><p className="text-lg text-slate-100 text-center">You become the most informed person in the room — and the power dynamic changes instantly.</p><div className="mt-8 p-4 rounded-xl bg-white/5 border border-white/10"><p className="text-sm text-slate-400 text-center italic">Individually, these are technicalities. <span className="text-white font-medium">Together, they are your leverage.</span></p></div></div></div>
        <div className="text-center text-slate-950"><Button asChild size="lg" variant="cta" className="group"><Link to="/sample-report">View a Real Sample AI Report<ArrowRight className="ml-2 w-5 h-5 transition-transform group-hover:translate-x-1" /></Link></Button><p className="mt-3 text-sm text-muted-foreground">See exactly how this intelligence looks before you upload anything.</p></div>
      </div>
    </section>;
}