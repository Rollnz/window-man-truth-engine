    ...
    paddingBottom: 'env(safe-area-inset-bottom)'
      ...
      <div className="px-4 py-3 space-y-1.5">
        ...
        <div className="flex gap-2">
          <Button asChild className="flex-1 h-12 bg-primary text-primary-foreground hover:bg-primary/90 font-semibold text-sm" onClick={() => handleCTAClick('beat_your_quote')}>
            <Link to={FOOTER_NAV.BEAT_YOUR_QUOTE}>
              <BadgeDollarSign className="w-4 h-4 mr-2" aria-hidden="true" />
              Beat Your Quote
            </Link>
          </Button>
          <Button asChild variant="outline" className="flex-1 h-12 bg-primary/10 text-primary border-primary/20 hover:bg-primary/15 font-semibold text-sm" onClick={() => handleCTAClick('scan_quote')}>
            <Link to={FOOTER_NAV.SCAN_QUOTE}>
              <ScanSearch className="w-4 h-4 mr-2 text-sidebar-primary" aria-hidden="true" />
              Scan Quote
            </Link>
          </Button>
        </div>
