...
export const QuoteUploadZone = forwardRef<HTMLDivElement, QuoteUploadZoneProps>(function QuoteUploadZone({
  ...
  return <div ref={ref} className="space-y-4">
        ...
        {/* Upload Zone */}
        <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={cn("relative rounded-xl border-2 border-dashed transition-all duration-300 overflow-hidden", "min-h-[300px] md:min-h-[400px]", isDragOver ? "border-primary bg-primary/5 scale-[1.02]" : "border-border hover:border-primary/50 bg-card/50", isAnalyzing && "pointer-events-none")}>
          {/* Analyzing State */}
          {isAnalyzing && <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/90 backdrop-blur-sm z-20">
          ...
          {imagePreview && !isAnalyzing && <div className="absolute inset-0">
          ...
          {showBeforeUploadOverlay && <>
              ...
              <FloatingCallout type="legal" label="Hidden Warranty Clause" className="top-4 left-0 md:top-6" />
              <FloatingCallout type="price" label="$2,400 Commission" className="top-16 right-0 md:top-20 md:right-0 left-auto border-l-0 border-r-2 rounded-l-md rounded-r-none pl-3 pr-3" hideMobile />
              <FloatingCallout type="missing" label="Missing Design Pressure" className="bottom-20 left-0 md:bottom-24" />
              <FloatingCallout type="warning" label="Vague Installation Terms" className="bottom-8 right-0 md:bottom-12 left-auto border-l-0 border-r-2 rounded-l-md rounded-r-none pl-3 pr-3" hideMobile />
            </>}

          {/* Upload CTA Overlay */}
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 z-10">
            {!imagePreview && !isAnalyzing && <div className="w-16 h-16 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center mb-4 backdrop-blur-sm">
        ...
        {/* Helper text */}
        <p className="text-center text-base text-[#bec1c6]">
