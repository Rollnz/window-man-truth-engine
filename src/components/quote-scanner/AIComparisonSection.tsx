...
.tp-circuit-bg{position:relative;background:linear-gradient(90deg,rgba(39,118,245,.08) 1px,transparent 1px),linear-gradient(0deg,rgba(39,118,245,.06) 1px,transparent 1px);background-size:48px 48px}
.tp-section::before{content:"";position:absolute;inset:0;pointer-events:none;opacity:.28;background:radial-gradient(circle at 20% 0%,rgba(39,118,245,.25),transparent 55%),radial-gradient(circle at 80% 100%,rgba(39,118,245,.22),transparent 60%)}
@keyframes tp-brain-pulse{0%,100%{transform:scale(1);box-shadow:0 0 16px rgba(39,118,245,.45),0 0 40px rgba(39,118,245,.35)}50%{transform:scale(1.04);box-shadow:0 0 24px rgba(39,118,245,.9),0 0 60px rgba(39,118,245,.7)}}
.tp-brain-shell{will-change:transform,box-shadow;animation:tp-brain-pulse 3s ease-in-out infinite}
@keyframes tp-brain-color-pulse{0%,100%{color:#e6effd;filter:drop-shadow(0 0 8px rgba(230,239,253,.5))}50%{color:#2776F5;filter:drop-shadow(0 0 20px rgba(39,118,245,.9))}}
...
.tp-node{position:absolute;width:6px;height:6px;border-radius:9999px;background:rgb(39 118 245);box-shadow:0 0 10px rgba(39,118,245,.9);will-change:transform,opacity;animation:tp-node-pulse 3.2s ease-in-out infinite}
.tp-node-rose{background:rgb(244 114 182);box-shadow:0 0 10px rgba(244,114,182,.9);animation-delay:1.6s}
.tp-glow-problem{border-color:rgba(248,113,113,.6);box-shadow:0 0 16px rgba(248,113,113,.25),inset 0 0 14px rgba(127,29,29,.35)}
.tp-glow-solution{border-color:rgba(39,118,245,.7);box-shadow:0 0 20px rgba(39,118,245,.32),inset 0 0 14px rgba(30,64,175,.35)}
...
@media(prefers-reduced-motion:reduce){.tp-brain-shell,.tp-stream-pulse,.tp-node,.tp-ping,.tp-brain-color-pulse,.shimmer-cta::after{animation:none!important;transform:none!important}.tp-brain-color-pulse{color:#2776F5!important;filter:drop-shadow(0 0 16px rgba(39,118,245,.8))}.tp-brain-shell{box-shadow:0 0 20px rgba(39,118,245,.6),0 0 50px rgba(39,118,245,.5)}.tp-node{opacity:.8}.tp-stream-pulse{opacity:.7}}
...
export const AIComparisonSection: React.FC<AIComparisonSectionProps> = ({ uploadRef }) => {
  ...
  return (
    <section className="tp-section bg-slate-950 py-16 md:py-24" aria-label="Why AI instead of human advisors">
      {/* Radial glow overlay handled by ::before */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="relative flex flex-col items-center gap-8 order-first lg:order-last">
              ...
              <div className="absolute inset-0 flex items-start justify-center pt-4 pointer-events-none">
                <div className="w-52 h-52 md:w-64 md:h-64 rounded-full border border-cyan-500/20 animate-[spin_20s_linear_infinite]" />
              </div>

              {/* Brain visual */}
              <div className="flex flex-col items-center gap-6">
                <div className="tp-brain-shell rounded-full bg-slate-900/80 border-2 border-cyan-500/30 p-8 md:p-10">
                  ...
                  <div className="rounded-xl border border-slate-700/60 bg-slate-900/60 backdrop-blur-sm p-3 space-y-2">
                    ...
                    <div className="relative h-1.5 rounded-full bg-slate-800 overflow-hidden">
                      {[0, 1, 2].map(i => (
                        <div
                          key={i}
                          className="tp-stream-pulse absolute inset-y-0 w-1/4 rounded-full bg-gradient-to-r from-cyan-500/80 to-blue-500/60"
                ...
                <button
                  ...
                  className="shimmer-cta inline-flex items-center gap-2 rounded-full bg-cyan-500 hover:bg-cyan-400 transition-colors px-6 py-3 text-sm font-semibold text-slate-900 shadow-lg shadow-cyan-500/30"
                >
            ...
            <div className="space-y-8">
              <div className="space-y-5">
                ...
                <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-1.5">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-wider text-cyan-300">AI Advantage</span>
                </div>

                <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight">
                  <span className="text-white">Unbiased Comparison. </span>
                  <span className="text-cyan-400">Data Driven insight. Updated Daily </span>
                </h2>
              ...
              <p className="text-[11px] uppercase tracking-widest text-cyan-500/60">
