...
export function SalesTacticsGuideModal({ isOpen, onClose, onSuccess }: SalesTacticsGuideModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      trackModalOpen({ modalName: 'sales_tactics_guide', sourceTool: 'sales-tactics-guide' });
    }
  }, [isOpen]);
    ...
    try {
      ...
      await supabase.functions.invoke('save-lead', {
        body: {
          ...
          aiContext: {
            source_form: 'sales-tactics-guide-upsell',
  ...
  const renderFormStep = () => (
    <>
      <div className="text-center mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900">11 Sales Tactics You Need to Know</h2>
      ...
      <div>
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="new" className="text-slate-700 cursor-pointer">New to me</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="updating" id="updating" />
            <Label htmlFor="updating" className="text-slate-700 cursor-pointer">One I'm updating</Label>
          </div>
        </RadioGroup>
      </div>
        ...
        <div className="space-y-2">
          {WINDOW_REASONS.map((reason) => (
            <div key={reason} className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor={reason} className="text-sm text-slate-700 cursor-pointer">{reason}</Label>
            </div>
          ))}
        </div>
      </div>
        ...
        >
          {TIMEFRAMES.map(({ value, label }) => (
            <div key={value} className="flex items-center space-x-2">
              ...
              <Label htmlFor={value} className="text-sm text-slate-700 cursor-pointer">{label}</Label>
            </div>
          ))}
        </RadioGroup>
      </div>
  ...
  const renderLocationStep = () => (
    <div className="space-y-5">
      ...
      <div>
        <Label htmlFor="remark" className="text-sm font-medium text-slate-700 mb-1 block">
