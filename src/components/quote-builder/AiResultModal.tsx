...
export const AiResultModal = ({ isOpen, onClose, title, content, isLoading }: AiResultModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/50 backdrop-blur-sm">
      <div className="bg-card shadow-2xl rounded-2xl max-w-2xl w-full max-h-[85vh] flex flex-col relative animate-in fade-in zoom-in duration-200 border border-border transition-colors">
        ...
        <div className="p-6 border-b border-border flex items-center gap-3">
          <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center border border-primary/20">
            {isLoading ? <Loader2 className="w-5 h-5 text-primary animate-spin" /> : <Sparkles className="w-5 h-5 text-primary" />}
          </div>
          <h3 className="text-xl font-bold text-foreground">{isLoading ? "Thinking..." : title}</h3>
        </div>
