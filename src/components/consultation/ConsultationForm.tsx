...
export function ConsultationForm({
  ...
  const handleWindowTypeChange = useCallback((type: WindowType, checked: boolean) => {
    setFormData(prev => {
      ...
      if (checked) {
        return {
          ...prev,
          windowTypes: [...current, type]
        };
      }
      return {
        ...prev,
        windowTypes: current.filter(t => t !== type)
      };
    });
  }, []);
  ...
  return <form ref={formRef} onSubmit={handleSubmit} className={cn('space-y-6', className)} noValidate>
        ...
        <CardContent className="pt-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              {errors.firstName && touched.firstName && <p id="firstName-error" className="text-sm text-destructive flex items-center gap-1">
            ...
            <div className="space-y-2">
              ...
              {errors.lastName && touched.lastName && <p id="lastName-error" className="text-sm text-destructive flex items-center gap-1">
                  <AlertCircle className="h-3 w-3" />
                  {errors.lastName}
                </p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.email && touched.email && <p id="email-error" className="text-sm text-destructive flex items-center gap-1">
                  <AlertCircle className="h-3 w-3" />
                  {errors.email}
                </p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.phone && touched.phone && <p id="phone-error" className="text-sm text-destructive flex items-center gap-1">
                  <AlertCircle className="h-3 w-3" />
                  {errors.phone}
                </p>}
            </div>
        ...
        <CardContent className="pt-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select value={formData.propertyType} onValueChange={value => handleChange('propertyType', value as PropertyType)}>
                  <SelectTrigger id="propertyType" className={cn('transition-colors', errors.propertyType && touched.propertyType && 'border-destructive')} aria-invalid={!!(errors.propertyType && touched.propertyType)}>
                ...
                {errors.propertyType && touched.propertyType && <p className="text-sm text-destructive flex items-center gap-1">
                    <AlertCircle className="h-3 w-3" />
                    {errors.propertyType}
                  </p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                {errors.cityZip && touched.cityZip && <p id="cityZip-error" className="text-sm text-destructive flex items-center gap-1">
                    <AlertCircle className="h-3 w-3" />
                    {errors.cityZip}
                  </p>}
              </div>
            ...
            <div className="space-y-2 max-w-xs">
              ...
              {errors.windowCount && touched.windowCount && <p id="windowCount-error" className="text-sm text-destructive flex items-center gap-1">
                  <AlertCircle className="h-3 w-3" />
                  {errors.windowCount}
                </p>}
            </div>
            ...
            <fieldset className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {WINDOW_TYPES.map(type => <div key={type.value} className="flex items-center space-x-2">
                    <Checkbox id={`window-${type.value}`} checked={formData.windowTypes?.includes(type.value)} onCheckedChange={checked => handleWindowTypeChange(type.value, checked as boolean)} />
                    <Label htmlFor={`window-${type.value}`} className="text-sm text-muted-foreground cursor-pointer">
                      {type.label}
                    </Label>
                  </div>)}
              </div>
            ...
            <fieldset className="space-y-3">
              ...
              <RadioGroup value={formData.impactRequired} onValueChange={value => handleChange('impactRequired', value as ImpactRequired)} className="flex flex-wrap gap-4">
                ...
                label: 'Not sure'
                    ...
                    <Label htmlFor={`impact-${option.value}`} className="text-sm text-muted-foreground cursor-pointer">
                      {option.label}
                    </Label>
                  </div>)}
              </RadioGroup>
              {errors.impactRequired && touched.impactRequired && <p className="text-sm text-destructive flex items-center gap-1">
                  <AlertCircle className="h-3 w-3" />
          ...
          <div className="space-y-6">
            ...
            <fieldset className="space-y-3">
              ...
              <RadioGroup value={formData.hasQuote} onValueChange={value => handleChange('hasQuote', value as QuoteStatus)} className="flex flex-wrap gap-4">
                ...
                label: 'In progress'
                    ...
                    <Label htmlFor={`quote-${option.value}`} className="text-sm text-muted-foreground cursor-pointer">
                      {option.label}
                    </Label>
                  </div>)}
              </RadioGroup>
              {errors.hasQuote && touched.hasQuote && <p className="text-sm text-destructive flex items-center gap-1">
                  <AlertCircle className="h-3 w-3" />
                  {errors.hasQuote}
                </p>}
            </fieldset>
            ...
            {formData.hasQuote === 'yes' && <fieldset className="space-y-3 animate-in slide-in-from-top-2 duration-200">
                ...
                label: '3+'
                      ...
                      <Label htmlFor={`quoteCount-${option.value}`} className="text-sm text-muted-foreground cursor-pointer">
                        {option.label}
                      </Label>
                    </div>)}
                </RadioGroup>
      ...
      <Card className="border-border bg-card shadow-sm">
        <CardContent className="pt-6">
          ...
          <RadioGroup value={formData.concern} onValueChange={value => handleChange('concern', value as ConcernType)} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {CONCERNS.map(concern => <div key={concern.value} className="flex items-center space-x-2">
                <RadioGroupItem value={concern.value} id={`concern-${concern.value}`} />
                <Label htmlFor={`concern-${concern.value}`} className="text-sm text-muted-foreground cursor-pointer">
                  {concern.label}
                </Label>
              </div>)}
          </RadioGroup>
      ...
      {/* Submission Error */}
      {submitError && <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-4 flex items-start gap-3">
      ...
      <div className="flex flex-col items-center gap-6">
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <Button type="submit" size="lg" disabled={isSubmitting || !isFormValid} className={cn('w-full sm:w-auto px-10 py-6 text-lg font-semibold transition-all duration-200', 'bg-primary hover:bg-primary/90', 'shadow-lg hover:shadow-xl hover:-translate-y-0.5', 'disabled:opacity-50 disabled:cursor-not-allowed')} style={{
          color: '#ffffff',
