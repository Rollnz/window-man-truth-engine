...
const pillars: Pillar[] = [{
  name: 'Safety & Code Match',
  score: 78,
  icon: ShieldCheck,
  color: 'bg-primary'
}, {
  name: 'Install & Scope Clarity',
  score: 42,
  icon: FileText,
...
function SegmentedBar({ score, color, isVisible }: { score: number; color: string; isVisible: boolean }) {
  ...
  return <div className="flex gap-1">
      {[...Array(segments)].map((_, i) => <div key={i} className={`h-3 flex-1 rounded-sm transition-all duration-500 ${i < filledSegments && isVisible ? color : 'bg-muted/50'}`} style={{
