...
export function SampleReportHeader({
  ...
  return <header className="fixed top-14 left-0 right-0 z-40 bg-background/95 backdrop-blur-sm border-b border-border/50">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {safeFirstName && safeFirstName.length >= 2 && <span className="shrink-0 text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full hidden sm:inline">
              Hi, {safeFirstName}
            </span>}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleTalkClick} className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
            <Phone className="w-4 h-4" />
            <span>Talk to Window Man</span>
          </button>

          {/* Primary: Upload My Estimate - Opens Lead Modal */}
          <Button variant="cta" size="sm" className="group" onClick={handleUploadClick}>
