...
export function PhoneCaptureModal({ isOpen, onClose, onSuccess }: PhoneCaptureModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess();
    } catch (error) { console.error('Phone capture error:', error); toast({ title: 'Unable to save', description: 'Please try again', variant: 'destructive' }); } finally { setIsLoading(false); }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <TrustModal className="sm:max-w-md" modalTitle="One More Step" modalDescription="To connect you with vetted partners, we need a phone number." headerAlign="center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="space-y-2"><Label htmlFor="phone" className="font-semibold text-slate-900">Phone Number</Label><Input id="phone" name="phone" type="tel" autoComplete="tel" placeholder="(555) 123-4567" value={phone} onChange={(e) => setPhone(formatPhoneNumber(e.target.value))} disabled={isLoading} autoFocus /></div>
          ...
          <Button type="submit" variant="cta" className="w-full" disabled={isLoading || phone.replace(/\D/g, '').length < 10}>{isLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</>) : (<><Check className="mr-2 h-4 w-4" />Continue to Partner Options</>)}</Button>
