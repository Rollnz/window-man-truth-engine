  ...
  return (
    <section ref={sectionRef} className="py-16 md:py-24 bg-background">
      <div className="container px-4">
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-3 gap-8">
                {steps.map((step, index) => { const Icon = step.icon; return (
                  <AnimateOnScroll key={step.number} delay={index * 150} duration={700}>
                ...
                {steps.map((step, index) => { const Icon = step.icon; return (
                  <AnimateOnScroll key={step.number} delay={index * 150} duration={700}>
                    <div className="relative flex gap-4">
                      ...
                      <div className="flex-1 bg-card border border-border/50 rounded-xl p-4 shadow-md shadow-black/5 dark:shadow-black/15 hover:shadow-lg transition-shadow duration-300"><div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Icon className="w-5 h-5 text-primary" /></div><h3 className="font-semibold text-foreground">{step.title}</h3></div><p className="text-sm text-muted-foreground">{step.description}</p></div>
                    </div>
                  </AnimateOnScroll>
                ); })}
              </div>
        ...
        <AnimateOnScroll duration={700} delay={500}>
