...
export function HowItWorksXRay({ onScanClick }: HowItWorksXRayProps) {
  return (
      ...
      <div className="container relative px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {STEPS.map((step, index) => (
            <Card 
              key={step.number}
              className={cn(
                "relative bg-slate-900/50 backdrop-blur-sm border p-6 transition-all duration-300 hover:scale-105 hover:shadow-lg group",
                step.borderColor,
                step.highlight && "ring-1 ring-cyan-500/20"
              )}
            >
              {/* Step number */}
              <div className={cn(
                "text-6xl font-black opacity-10 absolute top-4 right-4",
                step.highlight ? "text-cyan-400" : "text-slate-600"
              )}>
                {step.number}
              </div>
        ...
        <div className="text-center">
          <Button
            onClick={onScanClick}
            size="lg"
            className="px-8 py-6 text-lg font-bold bg-gradient-to-r from-cyan-500 to-cyan-400 hover:from-cyan-400 hover:to-cyan-300 text-slate-900 rounded-xl shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all"
          >
          ...
          <p className="text-slate-500 text-sm mt-4">
            Join 8,400+ Florida homeowners who refused to overpay
          </p>
        </div>
      </div>
    </section>
