...
const variantStyles = {
  ...
  minimal: {
    container:
      "bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 divide-slate-200 dark:divide-slate-700",
    icon: "text-primary",
  ...
  homepage: {
    container:
      "bg-sky-50 dark:bg-slate-800/90 border-sky-200 dark:border-slate-700 divide-sky-200 dark:divide-slate-700 shadow-sm",
...
export function UrgencyTicker({
  ...
  return (
    <div ref={ref} className={cn("shadow-none flex items-end justify-center border-0 border-[#e3e3e3]/0", className)}>
      <div
        className={cn(
        ...
        {showToday && (
          <div className={cn("flex items-center gap-1.5 sm:gap-2 h-full flex-shrink-0", sizes.padding, styles.todayBg)}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2">
              ...
              {showPermitFlags && (
                <span className={cn("font-medium tabular-nums", sizes.label, styles.todayText)}>
                  {permitFlagsCount} flagged for no permits
                </span>
              )}
            </div>
          </div>
