...
export function TestimonialCards({
  ...
  const sectionClasses = cn("py-16 md:py-24 px-4", isDark ? "bg-slate-950" : "bg-background", className);
  const cardClasses = cn("p-6 h-full transition-all duration-300", isDark ? "bg-slate-900/80 border-slate-700/50 hover:border-emerald-500/30" : "bg-card border-border hover:border-primary/40");
  ...
  if (!isInView) {
    return <section ref={sectionRef} className={cn("py-16 md:py-24 px-4 min-h-[500px]", isDark ? "bg-slate-950" : "bg-background")} aria-label="Testimonials loading" />;
  }
  return <section ref={sectionRef} className={cn(sectionClasses, "transition-all duration-700 ease-out", hasAnimated ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")}>
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="text-center mb-10">
          <Badge variant="outline" className={cn("mb-4 px-3 py-1", isDark ? "border-emerald-500/30 text-emerald-400" : "border-primary/30 text-primary")}>
            ⭐ Verified Homeowner Reviews
          </Badge>
          
          <h2 className={cn("text-3xl md:text-4xl lg:text-5xl font-black mb-4 leading-tight", textClasses)}>
            Real Savings.{' '}
        ...
        {/* Carousel */}
        <div className="relative px-4 md:px-12 shadow-lg" onMouseDown={handleCarouselInteraction} onTouchStart={handleCarouselInteraction}>
          <Carousel opts={{
          align: 'start',
          loop: true
        }} plugins={autoplayPlugin} className="w-full">
            <CarouselContent className="-ml-4">
              {REVIEWS.map(review => <CarouselItem key={review.id} className="pl-4 basis-full md:basis-1/2 lg:basis-1/3">
                  <Card className={cardClasses}>
                    {/* Avatar + Name + Location */}
                    <div className="flex items-center gap-3 mb-4">
                      <Avatar className={cn("h-12 w-12 border-2", isDark ? "border-emerald-500/30" : "border-primary/30")}>
                    ...
                    <div className="flex items-center gap-1 mb-3">
                      {Array.from({
                    length: 5
                  }).map((_, i) => <Star key={i} className={cn("w-4 h-4 fill-current", isDark ? "text-emerald-400" : "text-primary")} />)}
                    ...
                    {/* Savings Badge */}
                    <div className={cn("flex items-center gap-2 pt-3 border-t", isDark ? "border-slate-700/50" : "border-border")}>
                      <div className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium", isDark ? "bg-emerald-500/10 text-emerald-400" : "bg-primary/10 text-primary")}>
                        <span className={cn("w-2 h-2 rounded-full animate-pulse", isDark ? "bg-emerald-400" : "bg-primary")} />
            ...
            <CarouselPrevious className={cn("-left-2 md:-left-12 h-8 w-8", isDark ? "bg-slate-800 border-slate-700 text-white hover:bg-slate-700" : "")} />
            <CarouselNext className={cn("-right-2 md:-right-12 h-8 w-8", isDark ? "bg-slate-800 border-slate-700 text-white hover:bg-slate-700" : "")} />
          </Carousel>
        </div>
      </div>
    </section>;
