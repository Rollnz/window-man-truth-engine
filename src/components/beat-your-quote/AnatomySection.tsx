...
export function AnatomySection({
  ...
  const exposedBloat = BLOAT_ITEMS.filter(item => dissolvedCards.has(item.id)).reduce((sum, item) => sum + item.amount, 0);
  ...
  return <section id="beat-quote" ref={sectionRef} className="relative py-20 px-4 bg-[#0A0F14]">
      ...
      <div className="container max-w-4xl mx-auto">
        ...
        <BloatExposedCounter amount={exposedBloat} isVisible={dissolvedCards.size > 0 && !allCardsDissolved} />

        {/* Bloat Cards - All rendered, each handles its own dissolution */}
        <div className="space-y-4">
          {BLOAT_ITEMS.map(item => <BloatCard key={item.id} id={item.id} title={item.title} percentage={item.percentage} amount={item.amount} description={item.description} onDissolveChange={handleDissolveChange} />)}
        </div>
        ...
        <div className={`mt-12 transition-all duration-700 ${allCardsDissolved ? 'scale-105' : ''}`}>
          <RemainsBreakdown materials={12400} labor={4800} permit={1000} realPrice={REAL_PRICE} totalBloat={TOTAL_BLOAT} bloatPercentage={BLOAT_PERCENTAGE} isVisible={true} />
        </div>

        {/* Outcome Folders */}
        <div className="mt-16">
          <OutcomeFolders isVisible={allCardsDissolved} triggerCount={modalTriggerCount} />
        </div>
      </div>
    </section>;
}